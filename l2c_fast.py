...
class L2CFastEmbedding(Layer):
    ...
    def __init__(self, num_senses, vocab_dim, vector_dim, input_dim, output_dim = 1, init = 'uniform', activation = 'linear', **kwargs):
        ...
        m = np.zeros([vocab_dim,self.num_senses+1])
        m[:,0] = 1
        self.sense_vocab = theano.shared(m)
    ...
    def get_best_sense(self, word, curr_sense, context_vector, W_s):
        ...
        self.sense_vocab = T.set_subtensor(self.sense_vocab[word, new_sense+1], 1)
        return new_sense
    ...
    def disambiguate_context(self, word, context, W_g, W_s):
        ...
        word_sense = disambiguated_senses[disambiguated_senses.shape[0]/2]
        return disambiguated_senses, word_sense

    def sample(self,vec):
        out = T.eq(vec, 1).nonzero()[0]
        ran = RandomStreams(seed = np.random.randint(0,10))
        i = ran.random_integers(high = out.shape[0]-1, ndim = 1)
        return vec[0] -1

    def sample_contexts(self, word, context, W_g, W_s):
        context_senses, ignore_updates = theano.scan(lambda context_word: self.sample(self.sense_vocab[context_word]), sequences=[context])
        context_vector = K.sum(theano.scan(get_vector, sequences=[context, context_senses], non_sequences=[W_g, W_s])[0])
        sense_of_word = self.get_best_sense(word, -1, context_vector, W_s)
    ...
    def call(self, x, mask = None):
        ...
        def final_loss(word, is_pos, context):
            right_senses, word_sense = T.switch(T.eq(is_pos, 1), self.disambiguate_context(word, context, W_g, W_s), self.sample_contexts(word, context, W_g, W_s))
        ...
        return self.activation(theano.scan(final_loss, sequences=[x[:,0], x[:,1], x[:,2:]])[0])
